...
const FarmerWrapper = ({ lang, setLang }) => {
  ...
  useEffect(() => {
    const hasSelectedLang = localStorage.getItem('farmer_lang_modal_shown');
    if (!hasSelectedLang) {
      setShowLangModal(true);
      localStorage.setItem('farmer_lang_modal_shown', 'true');
    }
  }, []);
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: '0' }}>
        <div style={{ height: '65px', background: 'white', borderBottom: '1px solid #e0e0e0', display: 'flex', alignItems: 'center', padding: isMobile ? '0 10px' : '0 20px', justifyContent: 'space-between', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '10px' : '15px' }}>
            ...
            <h3 style={{ margin: 0, color: 'var(--primary-green)', fontSize: isMobile ? '16px' : '18px', fontWeight: '800', letterSpacing: '0.5px', maxWidth: isMobile ? '55vw' : 'none', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>KRISHIPATH - Kisan View</h3>
          </div>
